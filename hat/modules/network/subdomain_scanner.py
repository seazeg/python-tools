...
async def generate_subdomains(domain: str) -> list[str]:
    ...
    # 从字典文件加载
    wordlist_path = Path(__file__).parent / 'wordlists' / 'subdomain_basic.txt'
    try:
        ...
        # 使用内置的基础列表作为备选
        subdomains.update([
            'www', 'mail', 'admin', 'test', 'dev',
            'api', 'stage', 'prod', 'uat', 'beta'
        ])
    ...
    # 添加数字组合
    for prefix in ['test', 'dev', 'srv', 'server', 'host', 'web']:
